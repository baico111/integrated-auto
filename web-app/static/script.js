    ...
    fetch('/api/files', {
        ...
        .then(res => {
            if (res.success) {
                // 保存成功，开始执行
                clearRunningLogs();
                document.getElementById('runningLogModal').style.display = 'block';
                executeAndStreamLogs(folder, filename);

                // 自动打开 VNC 查看执行过程
                window.open('/vnc/vnc.html?path=vnc/websockify', '_blank');
            } else {
                alert('保存失败: ' + res.error);
            }
        })
...
function executeAndStreamLogs(folder, filename) {
    const logContent = document.getElementById('runningLogContent');
    logContent.textContent = '🚀 正在启动脚本...\n';
    ...
    fetch('/api/scripts/execute', {
        ...
        .catch(e => {
            logContent.textContent += `❌ 连接失败: ${e}\n`;
        });
}

function clearRunningLogs() {
    const logContent = document.getElementById('runningLogContent');
    if (logContent) logContent.textContent = '等待执行...\n';
}
