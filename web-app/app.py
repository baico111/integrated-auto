...
def get_desktop_env():
    ...
    # --- 终极注入：强制 SeleniumBase 使用本地驱动，禁止联网下载 ---
    env['SELENIUMBASE_CHROME_DRIVER_AUTO_DOWNLOAD'] = 'False'
    env['SELENIUMBASE_GECKO_DRIVER_AUTO_DOWNLOAD'] = 'False'
    env['SELENIUMBASE_CHROME_DRIVER_VERSION'] = '132.0.6834.110'
    env['UC_DRIVER_PATH'] = '/home/headless/.seleniumbase/drivers/uc_driver'
...
def execute_python_script(task_name, script_path):
    ...
    try:
        # === 1. Pre-Scan for Bypass Header ===
        bypass_url = None
        bypass_proxy = None # Future extension
        
        try:
            with open(script_path, 'r', encoding='utf-8', errors='ignore') as f:
                first_line = f.readline().strip()
                if first_line.startswith('#') and 'BYPASS_URL=' in first_line:
                    # Format: # BYPASS_URL=https://example.com
                    parts = first_line.split('BYPASS_URL=', 1)
                    if len(parts) > 1:
                        bypass_url = parts[1].strip()
        except Exception as e:
            logger.error(f"Failed to scan script header: {e}")

        # === 2. Execute Bypass (If needed) ===
        if bypass_url:
            print(f"🛡️ Detected Bypass Request: {bypass_url}")
            log_msg += f"--- Starting Cloudflare Bypass ---\nTarget: {bypass_url}\n"
            
            # Temp cookie file
            cookie_file = f"/tmp/cookies_{int(time.time())}_{os.path.basename(script_path)}.json"
            
            # Call bypass service
            bp_cmd = [sys.executable, '/app/web-app/bypass_service.py', bypass_url, cookie_file]
            # Adjust path if needed (we are in /app/web-app usually, but let's be safe)
            if not os.path.exists('/app/web-app/bypass_service.py'):
                # Fallback relative to current app.py
                bp_script = os.path.join(os.path.dirname(__file__), 'bypass_service.py')
                bp_cmd = [sys.executable, bp_script, bypass_url, cookie_file]
                
            print(f"Running bypass service: {bp_cmd}")
            bp_res = subprocess.run(bp_cmd, capture_output=True, text=True, timeout=120, env=env)
            
            log_msg += bp_res.stdout + "\n" + bp_res.stderr + "\n"
            
            if bp_res.returncode == 0 and os.path.exists(cookie_file):
                log_msg += "✅ Bypass Successful. Injecting CF_COOKIES_FILE.\n\n"
                env['CF_COOKIES_FILE'] = cookie_file
            else:
                log_msg += "❌ Bypass Failed. Continuing anyway...\n\n"
        
        # === 3. Run User Script ===
        cmd = [sys.executable, script_path]
        print(f"Running (User Script): {cmd}")
        ...
        # Cleanup
        if bypass_url and 'cookie_file' in locals() and os.path.exists(cookie_file):
...
def execute_script_realtime():
    ...
    # 解析完整路径
    target_dir = get_target_dir(folder)
    full_path = str(target_dir / script_path)
    
    if not os.path.exists(full_path):
        return jsonify({'error': f'脚本文件不存在: {full_path}'}), 404
    
    def generate():
        env = get_desktop_env()
        yield f"data: 🚀 开始执行: {script_path}\n\n"
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                yield f"data: ❌ 脚本执行失败 (退出码: {process.returncode})\n\n"
                
        except Exception as e:
            yield f"data: ❌ 执行异常: {str(e)}\n\n"
