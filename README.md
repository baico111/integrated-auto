...
### 4. 🛡️ Cloudflare 绕过 (UC Mode)

本版本集成了 Cloudflare Turnstile 自动绕过功能。
只需在你的 Python 脚本**第一行**添加以下标记：

```python
# BYPASS_URL=https://target-url.com
from selenium import webdriver
import os
import json

# 系统会自动绕过 CF 并将 Cookie 保存到环境变量指定的路径
cookie_file = os.environ.get('CF_COOKIES_FILE')

if cookie_file:
    with open(cookie_file) as f:
        cookies = json.load(f)['cookies']
    print("已获取 Cookies:", len(cookies))
```
