// ==UserScript==
// @name         Angara Host - 自动挂机防掉线助手
// @namespace    http://tampermonkey.net/
// @version      1.5
// @description  自动检测 AFK 状态，模拟心跳防止掉线，并在掉线时自动重登 (v1.5 增加 Session Error 处理)
// @author       Antigravity
// @match        *://*.worldofangara.fun/*
// @run-at       document-end
// @grant        none
// ==/UserScript==

(function () {
    'use strict';

    // ================= 配置区域 =================
    const CONFIG = {
        AFK_URL: '/earn/afk',      // 挂机页面的精准路径
        CHECK_INTERVAL: 10000,     // 状态检查频率 (10秒)
        HEARTBEAT_INTERVAL: 60000, // 心跳频率 (1分钟)
    };

    console.log("%c[Angara AFK Helper] 脚本已启动", "color: #ff4500; font-weight: bold;");

    // --- 状态面板 (参考用户喜好的右下角样式) ---
    const infoBox = document.createElement('div');
    Object.assign(infoBox.style, {
        position: 'fixed', bottom: '20px', right: '20px', padding: '12px 16px',
        backgroundColor: 'rgba(0, 0, 0, 0.9)', color: '#fff', borderRadius: '8px',
        zIndex: '999999', fontFamily: 'monospace', fontSize: '12px',
        borderLeft: '5px solid #00ff00', boxShadow: '0 4px 15px rgba(0,0,0,0.6)',
        minWidth: '260px', pointerEvents: 'none'
    });
    infoBox.innerHTML = '脚本 v1.5 初始化中...';
    document.body.appendChild(infoBox);

    // --- 状态更新函数 ---
    function updateHUD(color, status, detail, action) {
        infoBox.style.borderLeftColor = color;
        infoBox.innerHTML = `
            <div style="font-size:14px; font-weight:bold; color:${color}; margin-bottom:6px;">🚀 ${status}</div>
            <div style="color:#bbb; margin-bottom:4px;">📊 状态: <span style="color:#fff">${detail}</span></div>
            <div style="color:#888; border-top:1px solid #444; padding-top:4px; font-size:11px;">
               🤖 动作: <span style="color:${color}">${action}</span>
            </div>
        `;
    }

    // --- 点击触发函数 ---
    function triggerClick(element, name) {
        if (!element) return;
        updateHUD("#ffa500", "触发点击", name, "正在模拟点击动作...");
        const opts = { bubbles: true, cancelable: true, view: window };
        element.dispatchEvent(new MouseEvent('mousedown', opts));
        element.dispatchEvent(new MouseEvent('mouseup', opts));
        element.click();
    }

    // --- 核心心跳 (维持会话) ---
    setInterval(() => {
        window.scrollBy(0, 2);
        setTimeout(() => window.scrollBy(0, -2), 500);
        console.log("Keep-alive Heartbeat sent.");
    }, CONFIG.HEARTBEAT_INTERVAL);

    // --- 主循环 ---
    function mainLoop() {
        const currentUrl = window.location.href;
        const bodyText = document.body.innerText;

        // 0. 弹窗错误处理 (Session Error 等)
        const modalOkBtn = Array.from(document.querySelectorAll('button')).find(b =>
            (b.innerText === 'OK' || b.innerText === 'Confirm') &&
            (bodyText.includes('Session Error') || bodyText.includes('expired'))
        );
        if (modalOkBtn) {
            updateHUD("#ff0000", "检测到错误弹窗", "正在尝试重置会话...", "点击 OK 按钮");
            modalOkBtn.click();
            setTimeout(() => location.reload(), 2000);
            return;
        }

        // 1. 登录页检测 (加强匹配)
        if (currentUrl.includes('login') || bodyText.includes('Please log in again')) {
            updateHUD("#ff4500", "登录失效", "当前处于登录页面", "请手动输入账号密码登录");
            return;
        }

        // 2. 挂机页跳转检测
        if (!currentUrl.includes(CONFIG.AFK_URL)) {
            updateHUD("#ffa500", "不在挂机页", "准备跳转到 AFK 页面", "自动跳转中...");
            const afkLink = Array.from(document.querySelectorAll('a')).find(a => a.innerText.includes('AFK Rewards'));
            if (afkLink) {
                afkLink.click();
            } else if (!currentUrl.includes('/earn')) {
                window.location.pathname = CONFIG.AFK_URL;
            }
            return;
        }

        // 3. 挂机按钮检测
        const startBtn = Array.from(document.querySelectorAll('button')).find(b =>
            b.innerText.toLowerCase().includes('start afk') ||
            b.innerText.toLowerCase().includes('begin afk')
        );

        const statusText = document.body.innerText.toLowerCase();
        const isActive = statusText.includes('status: active') || statusText.includes('earning coins');

        if (isActive) {
            updateHUD("#00ff00", "运行中", "AFK Status: Active", "静默监控中 (每60s心跳)");
        } else if (startBtn) {
            updateHUD("#ff00ff", "挂机停止", "发现开始按钮", "点击 Start AFK...");
            triggerClick(startBtn, "Start AFK 按钮");
        } else {
            updateHUD("#888", "分析中", "等待关键元素加载", "正在扫描页面内容...");
        }
    }

    setInterval(mainLoop, CONFIG.CHECK_INTERVAL);
    console.log("Angara Host Helper v1.5 已启动");

})();
